...
export default function LoginPage({ onLoginSuccess }) {
  ...
  return (
    <div 
      ...
      className={`min-h-screen relative flex items-center justify-center p-4 sm:p-6 transition-colors duration-500 selection:bg-indigo-500/30 selection:text-indigo-300 font-sans ${
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        {isDark ? (
          ...
          <>
            <div className="absolute top-0 left-1/3 w-[600px] h-[600px] bg-indigo-500/10 rounded-full blur-[140px]" />
      ...
      <div className="relative w-full max-w-[440px] z-10 animate-in fade-in zoom-in-95 duration-300">
        <div className={`rounded-3xl p-8 sm:p-9 border shadow-2xl backdrop-blur-xl transition-all duration-300 ${
          ...
          <button
            ...
            className={`w-full mb-5 py-2.5 px-4 rounded-2xl text-xs font-bold transition-all duration-200 flex items-center justify-center gap-2.5 border shadow-sm disabled:opacity-50 group cursor-pointer ${
              isDark 
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <div>
              <label className={`block text-[11px] font-bold uppercase tracking-wider mb-1.5 flex items-center justify-between ${
                ...
                {touched.id && (
                  <span className={`text-[10px] font-semibold ${isIdentifierValid ? 'text-indigo-400' : 'text-rose-500'}`}>
              ...
              <div className="relative">
                <Mail className={`w-4 h-4 absolute left-3.5 top-3 transition-colors ${
                  focusedField === 'id' ? 'text-indigo-400' : isDark ? 'text-slate-500' : 'text-slate-400'
                }`} />
                <input
                  type="text"
                  required
                  ...
                  className={`
                    ...
                    ${isDark 
                      ? 'bg-slate-950 border-slate-800 text-white placeholder-slate-600 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/20' 
                      : 'bg-slate-50 border-slate-300 text-slate-900 placeholder-slate-400 focus:border-indigo-600 focus:ring-1 focus:ring-indigo-600/20'
                    }
            ...
            <div>
              <label className={`block text-[11px] font-bold uppercase tracking-wider mb-1.5 flex items-center justify-between ${
                ...
                {touched.pwd && (
                  <span className={`text-[10px] font-semibold ${isPasswordValid ? 'text-indigo-400' : 'text-rose-500'}`}>
              ...
              <div className="relative">
                <Lock className={`w-4 h-4 absolute left-3.5 top-3 transition-colors ${
                  focusedField === 'pwd' ? 'text-indigo-400' : isDark ? 'text-slate-500' : 'text-slate-400'
                }`} />
                <input
                ...
                <button
                  ...
                  className={`absolute right-3.5 top-2.5 transition-colors cursor-pointer ${
            ...
            <button
              ...
              className="w-full mt-2 py-3 px-4 bg-gradient-to-r from-indigo-500 via-indigo-600 to-purple-600 hover:from-indigo-400 hover:to-purple-500 text-white font-extrabold rounded-2xl text-xs transition duration-200 flex items-center justify-center gap-2 shadow-lg shadow-indigo-500/25 disabled:opacity-50 active:scale-[0.99] cursor-pointer"
            >
              {loading ? (
